new Vue({
	...
	methods: {
		ajaxs: function(page) {
			...
			$.ajax({
				type: "post",
				url: getIp() + "train/courseservice/list",
				data: {
					page: page,
					limit: 10
				},
...
//				var sort = $("#sort").val();
				...
				$.ajax({
					type: 'POST',
					url: getIp() + "train/courseservice/save",
					dataType: 'json',
					data:contentup,
...
//						var uploadinfo = JSON.parse(data);
		...
		modifythis(id) {
			...
			$.ajax({
				type: 'POST',
				url: getIp() + "train/courseservice/info/" + id,
				xhrFields: {
					withCredentials: true
				},
				beforeSend: function(xhr){
				...
				success: function(data) {
					...
					vm.modifyinfo = JSON.parse(data).courseService;
					console.log(vm.modifyinfo);
					vm.ID = JSON.parse(data).courseService.id;
...
//			var modysort = $("#modysort").val();
			...
			$.ajax({
				type: 'POST',
				url: getIp() + 'train/courseservice/update',
				contentType: "application/json",
				data: contentmod,
				xhrFields: {
					withCredentials: true
		...
		del(id) {
			...
			if(confir){
				$.ajax({
					type: 'POST',
					url: getIp() + 'train/courseservice/delete',
					dataType: 'json',
		...
		isshow(id) {
			...
			$.ajax({
				type: 'POST',
				url: getIp() + 'train/courseservice/isShow/' + id,
				xhrFields: {
					withCredentials: true
				},
				beforeSend: function(xhr){
