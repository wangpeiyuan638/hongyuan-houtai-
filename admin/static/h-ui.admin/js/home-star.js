new Vue({
	...
	methods: {
		ajaxs:function(page) {
			...
			let vm = this;
//			var sort = $('#sort').html();
			var mydata = {page: page,limit: 10};
			console.log(mydata);
			$.ajax({
				type: "post",
				url: getIp() + "/train/student/list",
				data: mydata,
				xhrFields: {
		...
		add() {
			let vm = this;
				//确定增加文本类请求
				var  iname = $("#starname").val();
				var  introduction = $('#starintroduction').val();
//				var  sort = $("#staraddress").val();
				var content = {
					iname: iname,
...
//						var uploadinfo = JSON.parse(data);
		...
		modifythis(id) {
			...
			$.ajax({
				type: 'POST',
				url: getIp() + "train/student/info/" + id,
				xhrFields: {
					withCredentials: true
				},
				beforeSend: function(xhr){
				...
				success: function(data) {
					...
					vm.modifyinfo = JSON.parse(data).student;
					console.log(vm.modifyinfo);
					vm.ID = JSON.parse(data).student.id;
					console.log(vm.ID);
...
//			var sort = $("#schooladdress").val();
			...
			$.ajax({
				type: 'POST',
				url: getIp() + 'train/student/update',
				contentType:"application/json",
				data:contentmod,
				xhrFields: {
					withCredentials: true
		...
		del(id) {
			...
			if(confir){
				$.ajax({
					type: 'POST',
					url: getIp() + 'train/student/delete',
					dataType: 'json',
		...
		isshow(id) {
			...
			$.ajax({
				type: 'POST',
				url: getIp() + 'train/student/isShow/' + id,
				xhrFields: {
					withCredentials: true
				},
				beforeSend: function(xhr){
